...
private struct CellButtonView: View {
    ...
    var body: some View {
        switch data {
        case .edit:
            getView(for: "pencil", title: "Edit", color: Color.superWhite)
                .background(Color.simpleOrange)   //if needed add all space in button filled color
        case .delete:
            getView(for: "trash", title: "Delete", color: Color.superWhite)
    ...
    func body(content: Content) -> some View {
        ZStack {
            content
                ...
                .offset(x: offset)
                .gesture(DragGesture(minimumDistance: 5, coordinateSpace: .local) //15
                    .onChanged({ (value) in
